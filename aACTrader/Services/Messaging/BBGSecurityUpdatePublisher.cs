...
{
    public class BBGSecurityUpdatePublisher
    {
        private readonly ILogger<BBGSecurityUpdatePublisher> _logger;
        private IConfiguration _configuration { get; set; }

        private const string QUEUENAME = "BBGSecurityUpdateQueue";

        private IModel _model { get; set; }
        private IConnection _connection { get; set; }

        public BBGSecurityUpdatePublisher(ILogger<BBGSecurityUpdatePublisher> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        public void PublishMessage(BBGSecurity securityReq)
        {
            try
            {
                ...
                String jsonified = JsonConvert.SerializeObject(securityReq);
                byte[] buffer = Encoding.UTF8.GetBytes(jsonified);
                _model.BasicPublish("", QUEUENAME, basicProperties, buffer);
            }
            catch (Exception e)
            {
                _logger.LogError(e, "Error publishing message to queue");
            }
        }
    }
}