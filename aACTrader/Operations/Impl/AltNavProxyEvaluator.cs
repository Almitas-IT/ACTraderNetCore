...
{
    public class AltNavProxyEvaluator
    {
        private readonly ILogger<AltNavProxyEvaluator> _logger;
        private readonly ExpressionContext _context;
        private static readonly DateTime TodaysDate = DateTime.Now.Date;

        public AltNavProxyEvaluator(ILogger<AltNavProxyEvaluator> logger)
        {
            _logger = logger;
            ...
            , IDictionary<string, string> priceTickerMap)
        {
            try
            {
                ...
                foreach (FundProxy fundProxy in fundProxyFormula.ProxyTickersWithCoefficients)
                {
                    ...
                    else
                    {
                        ...
                        if (securityPrice != null)
                        {
                            ...
                            etfRtn = securityPrice.MidPrcChng.GetValueOrDefault();
                            if (histETFReturns != null && histETFReturns.TryGetValue(etfTicker, out double? etfHistRtn))
                                etfRtn = (1.0 + etfRtn) * (1.0 + etfHistRtn.GetValueOrDefault()) - 1.0;
                        }
                    }
                ...
                Nullable<double> proxyReturn = EvaluateExpression(fundProxyFormula.ProxyFormula, ticker);
                if (fundForecast.LastDvdAdjNav.HasValue && proxyReturn.HasValue)
                {
                    ...
                    if (!Double.IsNaN(proxyNav) && !Double.IsInfinity(proxyNav))
                        fundForecast.AltProxyNav = proxyNav;
                    if (!Double.IsNaN(proxyReturn.GetValueOrDefault()) && !Double.IsInfinity(proxyReturn.GetValueOrDefault()))
                        fundForecast.AltProxyRtn = proxyReturn;
                    if (!Double.IsNaN(proxyAdjReturn) && !Double.IsInfinity(proxyAdjReturn))
                        fundForecast.AltAdjProxyRtn = proxyAdjReturn;

                    fundForecast.AltProxyForm = fundProxyFormula.ProxyFormula;
                }
            }
            catch (Exception ex)
            {
        ...
        private Nullable<double> EvaluateExpression(string expression, string ticker)
        {
            ...
            try
            {
                IGenericExpression<double> eGeneric = _context.CompileGeneric<double>(expression.Replace(" ", "_"));
        ...
        public IList<FundProxy> GetProxyFormula(CachingService cache, string ticker)
        {
            ...
            IDictionary<string, FundProxyFormula> fundProxyFormulaDict = cache.Get<IDictionary<string, FundProxyFormula>>(CacheKeys.ALT_PROXY_FORMULAS);
            ...
            if (fundProxyFormulaDict.TryGetValue(ticker, out FundProxyFormula fundProxyFormula))
            {
                    ...
                    && fundForecastDict.TryGetValue(ticker, out FundForecast fundForecast))
                {
                    ...
                    IDictionary<string, FundETFReturn> proxyHistReturnsDict = cache.Get<IDictionary<string, FundETFReturn>>(CacheKeys.ALT_PROXY_ETF_RETURNS);
                    ...
                    foreach (FundProxy fundProxyWithCoefficients in fundProxyFormula.ProxyTickersWithCoefficients)
                    {
                        ...
                        else
                        {
                            ...
                            //if (securityPrice != null && securityPrice.MidPrcChng.HasValue)
                            {
                                //double etfRtn = securityPrice.PrcRtn.GetValueOrDefault();
                                double etfRtn = securityPrice.MidPrcChng.GetValueOrDefault();
                                double? etfHistRtn = 0;
