...
{
    public class SimNeovestOrderSummaryConsumer
    {
        private readonly ILogger<SimNeovestOrderSummaryConsumer> _logger;
        private CachingService _cache { get; set; }
        private IConfiguration _configuration { get; set; }

        private const string QUEUENAME = "OrderSummaryQueueSim";

        private IModel _model { get; set; }
        private IConnection _connection { get; set; }
        private long _consumedMessages;

        public SimNeovestOrderSummaryConsumer(ILogger<SimNeovestOrderSummaryConsumer> logger, CachingService cache, IConfiguration configuration)
        {
            _logger = logger;
            _cache = cache;
        ...
        public void ProcessMessage(object sender, BasicDeliverEventArgs e)
        {
            ...
            try
            {
                ...
                if (orderSummaryDict.TryGetValue(orderSummaryRecord.OrdId, out record))
                {
                    ...
                    if (orderSummaryRecord.IsAnExec.Equals("Y"))
                        record.TrdCumQty = CommonUtils.AddNullableInts(record.TrdCumQty, record.TrdQty);
                ...
                {
                    _logger.LogInformation("Adding NEW Order -> MainOrderId/OrderId/ReferenceId: " + orderSummaryRecord.MainOrdId + "/" + orderSummaryRecord.OrdId + "/" + orderSummaryRecord.RefId);
                ...
                {
                    IDictionary<string, OrderSummary> orderExecutionDetails = _cache.Get<Dictionary<string, OrderSummary>>(CacheKeys.SIM_ORDER_EXECUTION_DETAILS);
            ...
            {
                _logger.LogInformation("Consumed " + _consumedMessages + " Order Summary real-time messages so far...");
                _consumedMessages = 0;
            }
        }
        ...
        private void AddToOrderMap(OrderSummary order)
        {
            try
            {
                IDictionary<string, IDictionary<string, OrderSummary>> orderSummaryBySymbolDict = _cache.Get<IDictionary<string, IDictionary<string, OrderSummary>>>(CacheKeys.SIM_ORDER_SUMMARY_BY_SYMBOL);
                ...
                //Capture Ref Id to Order Id Map
                    ...
                    && batchOrdersByRefIdDict.TryGetValue(nvOrder.RefId, out NewOrder nOrder))
                {
                    ...
                    else if (nOrder.DiscountTarget.HasValue)
                    {
                        ...
                        if (fundForecast != null)
                        {
                            ...
                            almOrder.DscntToLastPr = DataConversionUtils.CalculateReturn(almOrder.OrdPr, fundForecast.EstNav);
                        }
                    }
                }
            }
