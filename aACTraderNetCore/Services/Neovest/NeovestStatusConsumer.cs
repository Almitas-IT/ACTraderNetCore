...
{
    public class NeovestStatusConsumer
    {
        private readonly ILogger<NeovestStatusConsumer> _logger;
        private CachingService _cache { get; set; }
        private IConfiguration _configuration { get; set; }

        private const string QUEUENAME = "NeovestStatusQueue";

        private IModel _model { get; set; }
        private IConnection _connection { get; set; }
        private long _consumedMessages;

        public NeovestStatusConsumer(ILogger<NeovestStatusConsumer> logger, CachingService cache, IConfiguration configuration)
        {
            _logger = logger;
            _cache = cache;
        ...
        public void ProcessMessage(object sender, BasicDeliverEventArgs e)
        {
            try
            {
                ...
                ServiceStatus record = JsonConvert.DeserializeObject<ServiceStatus>(jsonified);
            ...
            {
                _logger.LogInformation("Consumed " + _consumedMessages + " Neovest Status real-time messages...");
                _consumedMessages = 0;
            }
        }
    }
