...
{
    public class PortProxyEvaluator
    {
        private readonly ILogger<PortProxyEvaluator> _logger;
        private readonly ExpressionContext _context;
        private static readonly DateTime TodaysDate = DateTime.Now.Date;

        public PortProxyEvaluator(ILogger<PortProxyEvaluator> logger)
        {
            _logger = logger;
            ...
            , IDictionary<string, string> priceTickerMap)
        {
            try
            {
                ...
                if (proxyReturn.HasValue)
                {
                    if (!Double.IsNaN(proxyReturn.GetValueOrDefault()) && !Double.IsInfinity(proxyReturn.GetValueOrDefault()))
                        fundForecast.PortFIProxyRtn = proxyReturn;

                    fundForecast.PortFIProxyForm = fundProxyFormula.ProxyFormula;
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error evaluating Port Proxy Formula for ticker: " + ticker);
        ...
        public IList<FundProxy> GetProxyFormula(CachingService cache, string ticker)
        {
            ...
            IDictionary<string, FundProxyFormula> fundProxyFormulaDict = cache.Get<IDictionary<string, FundProxyFormula>>(CacheKeys.PORT_PROXY_FORMULAS);
